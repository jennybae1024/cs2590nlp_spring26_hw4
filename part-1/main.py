...
def create_augmented_dataloader(args, dataset):
    ...
    transformed_train = dataset["train"].map(custom_transform, load_from_cache_file=False, num_proc=4)
    transformed_tokenized = transformed_train.map(tokenize_function, batched=True, load_from_cache_file=False, num_proc=4)
    ...
    augmented_examples = transformed_tokenized.shuffle(seed=42).select(range(5000))

    augmented_train_dataset = concatenate_datasets([original_train, augmented_examples])
...
def create_transformed_dataloader(args, dataset, debug_transformation):
    ...
    transformed_dataset = dataset["test"].map(custom_transform, load_from_cache_file=False, num_proc=4)
